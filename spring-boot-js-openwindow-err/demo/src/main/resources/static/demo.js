...
function openWin(formName){
    ...
    var win = window.open('about:blank', formName);
...
function printOpenErrMsg(){
    ...
    if(errList){
        var errMessages = errList.getElementsByTagName('li');
        //子画面にエラーメッセージがある場合
        if(errMessages && errMessages.length > 0){
            //親画面のエラーメッセージリストを取得し初期化後、メッセージを追加
            var errListP = window.opener.document.getElementById('errList');
            if(errListP){
                errListP.innerHTML = "";
                errListUlP = document.createElement("ul");
                errListP.appendChild(errListUlP)
                for(var i = 0; i < errMessages.length; i++){
                    var msg = errMessages[i].textContent;
                    errListLiP = document.createElement("li");
